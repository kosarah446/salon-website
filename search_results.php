...
$mysqli->set_charset('utf8');
	...
	{
		$sql = $sql . " AND appointments.fname LIKE '%".$_GET["fname"]."%'";
	}

	if(isset($_GET["lname"]) && !empty($_GET["lname"]))
	{
		$sql = $sql . " AND appointments.lname LIKE '%".$_GET["lname"]."%'";
	}

	if(isset($_GET["email"]) && !empty($_GET["email"]))
	{
		$sql = $sql . " AND appointments.email LIKE '%".$_GET["email"]."%'";
	}

	if(isset($_GET["location"]) && !empty($_GET["location"]))
	{
		$sql = $sql . " AND appointments.location_id = '".$_GET["location"]."'";
	}

	if(isset($_GET["service"]) && !empty($_GET["service"]))
	{
		$sql = $sql . " AND appointments.service_id = '".$_GET["service"]."'";
	}
...
<form id="msform" class = "booking-results" form action="search_results.php" method="GET">
  <!-- progressbar -->
  ...
  <!-- fieldsets -->
  <fieldset>
  	<h2 class="fs-title">Booking Results</h2>
    ...
    <div class="row mt-3">
      ...
      <div class="col-8">
        ...
        if(isset($row['location']) && !empty($row['location'])){
          $sql_locations = "SELECT * FROM locations WHERE id = " . $row['location'] . ";";
    ...
    <div class="row mt-3">
      ...
      <div class="col-8">
        ...
        if(isset($row["service"]) && !empty($row["service"])){
            ...
            $sql_services = "SELECT * FROM services WHERE services.id = " . $row["service"]. ";";
...
?>
        ...
        <div class="row mt-3">
      <div class="col-4 text-right labels"></div>
      <div class="col-8">
      

<a href="delete.php?appointments.id=<?php echo $row['appointments.id']?>&title=<?php echo $row['fname']?>"class="btn btn-outline-danger delete-btn" onclick="return confirm('You are about to delete DVD <?php echo addslashes($row["fname"]) ?>');">Delete</a>
     ...
     <hr>
<?php endwhile; ?>
	<a href = "search_booking.php"><h3 class="link">Back to Search Form</h3></a>
  </fieldset>
</form> 
